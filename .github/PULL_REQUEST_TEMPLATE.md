Tasks list:

- [ ]
- [ ]
- [ ]

Definition of done:

- [ ] Create translation keys in French and listed alphabetically
- [ ] Check access to controllers (if concerned)
- [ ] Respect the UI Admin (if the feature concerns the Admin)
- [ ] Generate DB migration if the structure changes (`$ make migrations`)
- [ ] Re-check Acceptance test: proofreading the story at the developement end
- [ ] Unit tests pass
- [ ] Functionnal tests pass
- [ ] No errors on Insight
- [ ] No conflict with `master` or solve them as soon as possible.
- [ ] To be reviewed (have at least one "approve") in order to pass the feature to "preprod"
