...
Le forfait de participation (`Package`) est une entité associé à un type de participation qui permet de déterminer les produits achetables par l'utilisateur lors de son passage dans le tunnel d'achat.
...
La seconde étape permet de sélectionner pour chaque participant de la fiche, le produit de type `participant` qui lui sera associé. Certains produits ont des prix différents et offres des particularités différents (voir la rubrique [AvailabilityTimeRange](AvailabilityTimeRange.md) pour ces particularités).
...
Une fois ces trois étapes remplies, l'utilisateur doit remplir les informations de facturations qui permettront de déterminer si la commande est soumise à TVA ou non (en fonction de la configuration de l'événement, et du Numéro de TVA intracommunautaire et du pays mentionné dans les informations de facturation).

Enfin, l'étape de récapitalif de la commande, où l'utilisateur valide les conditions générales de vente ainsi que l'ajout de code promotionnel.

Enfin, l'utilisateur est dirigié sur l'étape de selection du moyen de paiement. Qui propose en fonction de la configuration de l'événement et du type de participation, la possibilité de payer par Paypal, virement ou chèque, et un accompte ou la totalié.
...
Il y a alors la possibilité pour l'admin d'annuler les commandes et ainsi autorisé de passer une nouvelle fois dans le tunnel d'achat.
