...
class AuthenticationFailureSubscriberTest extends TestCase
{
    ...
    {
        $this->blockingFail('main', Events::USER_ACCOUNT_TEMPORARILY_DISABLED, UserTemporarilyDisabledEvent::class);
    }

    public function testBlockingFailWithExistingAdmin()
    {
        $this->blockingFail('admin', Events::ADMIN_ACCOUNT_TEMPORARILY_DISABLED, AdminTemporarilyDisabledEvent::class);
    }

    private function blockingFail(string $firewall, string $dispatchedEventName, string $dispatchedEventClass)
    {
        $authenticationFailureEvent = $this->prophesize(AuthenticationFailureEvent::class);

        $token = $this->prophesize(UsernamePasswordToken::class);
        $token->getProviderKey()->shouldBeCalled()->willReturn($firewall);
        ...
        $this->eventDispatcher->dispatch(
            $dispatchedEventName,
            Argument::type($dispatchedEventClass)
        )->shouldBeCalled();
