...
class UpdateHandler
{
    ...
    public function handle(Update $update)
    {
        ...
        if ($update->message) {
            ...
            $this->events[] = [Events::MEETING_UPDATE_MESSAGE, new MessageEvent($message, $update->user)];
        }
    ...
    private function updateFromParticipants(Update $update)
    {
        ...
        foreach ($update->meeting->getFromParticipants() as $participant) {
            if (!in_array($participant, $update->participants)) {
                ...
                $this->events[] = [Events::MEETING_PARTICIPANT_REMOVED, new ParticipantRemovedEvent($update->user, $participant, $update->meeting, $update->message, $update->date)];
            }
        }
        ...
        foreach ($update->participants as $participant) {
            if (!$update->meeting->hasFromParticipant($participant)) {
                ...
                $this->events[] = [Events::MEETING_PARTICIPANT_ADDED, new ParticipantAddedEvent($update->user, $participant, $update->meeting, $update->message, $update->date)];
            }
        }
    }
