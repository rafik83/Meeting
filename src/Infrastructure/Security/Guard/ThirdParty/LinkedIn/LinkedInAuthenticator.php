...
class LinkedInAuthenticator extends SocialAuthenticator
{
    ...
    public function getCredentials(Request $request)
    {
        ...
        return [
            'accessToken' => $this->fetchAccessToken($this->getLinkedInClient()),
            'event' => $event,
            'locale' => $locale
        ];
    }
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        try {
            ...
            $linkedInResourceOwner = $this->getLinkedInClient()->fetchUserFromToken($credentials['accessToken']);

            return $this->getOrCreateUserHandler->handle(
                new GetOrCreateUser(
                    $credentials['event'],
                    $credentials['locale'],
                    $linkedInResourceOwner->getEmail(),
                    $linkedInResourceOwner->getFirstName(),
                    $linkedInResourceOwner->getLastName()
                )
            );
