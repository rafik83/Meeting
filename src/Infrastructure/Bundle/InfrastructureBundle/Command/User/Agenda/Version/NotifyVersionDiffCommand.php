...
class NotifyVersionDiffCommand extends Command
{
    private const NAME = 'vimeet:user:notify-agenda-version-diff';
    ...
    protected function configure()
    {
        $this
            ->setName(self::NAME)
            ->setDescription('Notify the users of version diff in their agenda for incoming events')
        ;
    }
    ...
    {
        $events = $this->eventRepository->getNotPastEventsWithAgendaPublished($this->dateTime);
        $eventsToNotify = [];

        foreach ($events as $event) {
            $firstDay = $event->getFirstDay();

            if ($firstDay->getBegin() < $this->dateTime) {
                continue;
            }
        ...
        $this->recurrentNotificationOfChangedInVersionCommandHandler->handle(
            new RecurrentNotificationOfChangedInVersionCommand($eventsToNotify, false)
        );
    }
}
