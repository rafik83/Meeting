...
Feature: Edit participant status
  ...
  Scenario: I can use filters, navigate on admin and see my filters was saved
    Given I am logged with "test@test.com" on admin
    And I am on this page "/admin/fr/event"
    When I go to "/admin/fr/event/1/sheet?type=1&page=1&validationState=draft&state=pending&enabled=1&order=createdAt"
    ...
    And I should see "form.sheet_filter.children.order.label"
    When I go to this page "/admin/fr/event"
    And I go to "/admin/fr/event/1/sheet"
    Then I should be on this page "/admin/fr/event/1/sheet?type=1&page=1&validationState=draft&state=pending&enabled=1&order=createdAt"
